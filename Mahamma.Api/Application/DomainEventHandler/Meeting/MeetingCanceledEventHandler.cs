...
namespace Mahamma.Api.Application.DomainEventHandler.Meeting
{
    public class MeetingCanceledEventHandler : INotificationHandler<MeetingCanceledEvent>
    {
        ...
        public MeetingCanceledEventHandler(IMediator mediator, INotificationResourceReader notificationResourceReader, INotificationService notificationService)
        {
            _mediator = mediator;
        ...
        public async Task Handle(MeetingCanceledEvent request, CancellationToken cancellationToken)
        {
        ...
        private NotificationDto PrepareNotification(string meetingTitle ,int meetingId, UserDto sender, long receiverUserId)
        {
            return new NotificationDto
            {
                ...
                NotificationTypeId = NotificationType.MeetingCanceled.Id,
                IsRead = false,
                NotificationTitleEnglish = GetNotificationTitle(NotificationType.MeetingCanceled.Id, Language.English.Id),
                NotificationBodyEnglish = string.Format("{0}" + GetNotificationBody(NotificationType.MeetingCanceled.Id, Language.English.Id) + "{1}", sender.FullName, meetingTitle),
                NotificationBodyArabic = string.Format("{0}" + GetNotificationBody(NotificationType.MeetingCanceled.Id, Language.Arabic.Id) + "{1}", sender.FullName, meetingTitle),
                NotificationTitleArabic = GetNotificationTitle(NotificationType.MeetingCanceled.Id, Language.Arabic.Id)
            };
        }
