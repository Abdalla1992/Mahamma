...
{
    public class MeetingAddedEventHandler : INotificationHandler<MeetingAddedEvent>
    {
        private readonly IMediator _mediator;
        private readonly INotificationService _notificationService;
        private readonly INotificationResourceReader _notificationResourceReader;

        public MeetingAddedEventHandler(IMediator mediator, INotificationResourceReader notificationResourceReader, INotificationService notificationService)
        {
            _mediator = mediator;
        ...
        public async Task Handle(MeetingAddedEvent request, CancellationToken cancellationToken)
        {
            #region Send Notification
            if (request.Meeting.Members?.Count > 0)
            {
                List<NotificationDto> notificationListDto = new();
                foreach (var member in request.Meeting.Members.Where(m => m.UserId != request.CreatorUser.Id))
                {
        ...
        private NotificationDto PrepareNotification(string meetingTitle ,int meetingId, UserDto sender, long receiverUserId)
        {
            return new NotificationDto
            {
                ...
                NotificationTypeId = NotificationType.MeetingAdded.Id,
                IsRead = false,
                NotificationTitleEnglish = GetNotificationTitle(NotificationType.MeetingAdded.Id, Language.English.Id),
                NotificationBodyEnglish = string.Format("{0}" + GetNotificationBody(NotificationType.MeetingAdded.Id, Language.English.Id) + "{1}", sender.FullName, meetingTitle),
                NotificationBodyArabic = string.Format("{0}" + GetNotificationBody(NotificationType.MeetingAdded.Id, Language.Arabic.Id) + "{1}", sender.FullName, meetingTitle),
                NotificationTitleArabic = GetNotificationTitle(NotificationType.MeetingAdded.Id, Language.Arabic.Id)
            };
        }
