...
{
    public class SubmitTaskCommandValidator : AbstractValidator<SubmitTaskCommand>
    {
        private readonly Microsoft.AspNetCore.Http.IHttpContextAccessor _httpContext;
        private readonly IMessageResourceReader _messageResourceReader;
        public SubmitTaskCommandValidator(Microsoft.AspNetCore.Http.IHttpContextAccessor httpContext, IMessageResourceReader messageResourceReader)
        {
            _httpContext = httpContext;
            _messageResourceReader = messageResourceReader;
            RuleFor(command => command.TaskId).GreaterThan(0).WithMessage(GetValidationMessage("ValidTaskId"));
            RuleFor(command => command.DurationInHours).GreaterThan(0).WithMessage(GetValidationMessage("InvalidDuration"));
        }
